...
export const Route = createFileRoute("/snapshots")({
  head: () => ({
    meta: [
      ...
      { name: "description", content: "Save the mode and phase you chose, with a note. Build a personal record of how your work moves." },
    ],
  }),
  ...
  return (
    <main className="relative min-h-screen overflow-hidden pb-32">
      ...
      <header className="relative z-10 mx-auto flex max-w-3xl items-center justify-between px-6 py-8">
        ...
        <Link to="/timing" className="text-xs tracking-[0.22em] uppercase text-muted-foreground hover:text-foreground transition-calm">
          Timing →
        </Link>
      </header>
      ...
      <section className="relative z-10 mx-auto max-w-3xl px-6 pt-8">
        ...
        <p className="mt-5 max-w-xl text-base leading-relaxed text-muted-foreground fade-up" style={{ animationDelay: "160ms" }}>
          Choose a mode, optionally a phase, add a line. The studio stores the labels
          you picked. Over time you'll see your own pattern — written by you.
        </p>
      </section>
