...
export const Route = createFileRoute("/reflections")({
  head: () => ({
    meta: [
      ...
      { name: "description", content: "A reflection space. You write; the studio holds. Nothing is interpreted." },
    ],
  }),
  ...
  return (
    <main className="relative min-h-screen overflow-hidden pb-32">
      ...
      <header className="relative z-10 mx-auto flex max-w-3xl items-center justify-between px-6 py-8">
        ...
        <Link to="/snapshots" className="text-xs tracking-[0.22em] uppercase text-muted-foreground hover:text-foreground transition-calm">
          Snapshots →
        </Link>
      </header>
      ...
      <section className="relative z-10 mx-auto max-w-3xl px-6 pt-8">
        ...
        <p className="mt-5 max-w-xl text-base leading-relaxed text-muted-foreground fade-up" style={{ animationDelay: "160ms" }}>
          A quiet surface for whatever is worth keeping. The studio will not respond,
          analyze, or interpret. The reflection is yours.
        </p>
      </section>
