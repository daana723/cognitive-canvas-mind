...
export const Route = createFileRoute("/map")({
  head: () => ({
    meta: [
      ...
      { name: "description", content: "Describe your current creative state in your own words. The studio holds it, never interprets it." },
    ],
  }),
  component: MapPage,
});
...
function MapPage() {
  ...
  useEffect(() => { setPhase(studioStore.load().currentPhase); }, []);
  ...
  return (
    <main className="relative min-h-screen overflow-hidden pb-32">
      ...
      <header className="relative z-10 mx-auto flex max-w-4xl items-center justify-between px-6 py-8">
        ...
        <Link to="/reflections" className="text-xs tracking-[0.22em] uppercase text-muted-foreground hover:text-foreground transition-calm">
          Reflections →
        </Link>
      </header>
      ...
      <section className="relative z-10 mx-auto max-w-3xl px-6 pt-8">
        ...
        <p className="mt-5 max-w-xl text-base leading-relaxed text-muted-foreground fade-up" style={{ animationDelay: "160ms" }}>
          Describe the work in your own words. Add threads as they arrive. Choose the
          phase that fits the story right now. No analysis is performed; the studio
          only stores what you put in.
        </p>
      </section>
