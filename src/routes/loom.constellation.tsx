    ...
    <section className="relative z-10 mx-auto max-w-6xl px-6 pt-8">
      <div className="mx-auto max-w-3xl text-center">
        ...
        <p className="mt-5 text-base leading-relaxed text-muted-foreground">
          Each agent is a view over the local Loom modules. The Loom orchestrates; the modules
          return structured creative artifacts.
        </p>
      </div>
            ...
            >
              <div className="flex items-start gap-4">
                ...
                >
                  <Sigil
                    variant={agent.sigil}
                    className={agent.id === "loom" ? "h-16 w-16" : "h-12 w-12"}
                  />
                </div>
                <div>
              ...
              <div className="mt-5 space-y-2">
                  ...
                  (module) =>
                    module && (
                      <Link
                        ...
                        className="block rounded-xl border border-border/50 p-3 transition-calm hover:border-primary/60"
                      >
                ...
                {modules.length === 0 && (
                  <Link
                    to={agent.id === "avatar" ? "/spark" : "/loom"}
                    className="block rounded-xl border border-border/50 p-3 text-sm text-muted-foreground transition-calm hover:border-primary/60"
                  >
                    {agent.id === "avatar"
                      ? "Avatar coherence is held through SPARK reflection for now."
                      : "The Loom owns orchestration rather than a single module."}
                  </Link>
                )}
              </div>
            </article>
