...
function LoomModulePage() {
  ...
  const agent = useMemo(
    () => LOOM_AGENTS.find((item) => item.moduleIds.includes(moduleId)),
    [moduleId],
  );
      ...
      <section className="relative z-10 mx-auto max-w-3xl px-6 pt-12">
        <div className="glass-panel rounded-2xl p-6 text-center">
          ...
          <p className="mt-3 text-sm text-muted-foreground">
            No Loom module is registered for {moduleId}.
          </p>
          <Link
            to="/loom/constellation"
            className="mt-5 inline-block text-[10px] tracking-[0.24em] uppercase text-thread"
          >
  ...
  const runCurrentModule = async () => {
    ...
    const result = await loomClient.run({
      ...
      context: { source: "loom-module-route" },
    });
    if (result.ok) {
      setRun(result.data);
