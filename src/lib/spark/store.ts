...
import { CURRENT_PROMPTS } from "./currentPrompts";
...
function loadResponses<T extends Record<string, Resonance>>(key: string, promptIds: string[]): T {
  ...
  // Remove legacy/mismatched data once it has been detected so returning users
  // are not repeatedly trapped in the broken resume path on later visits.
