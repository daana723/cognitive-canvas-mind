...
export const sparkResponsesStore = {
  load(): Responses { return read<Responses>(RESP_KEY, {}); },
  save(r: Responses) { write(RESP_KEY, r); },
  clear() { if (isBrowser()) localStorage.removeItem(RESP_KEY); },
};

export const currentResponsesStore = {
  load(): CurrentResponses { return read<CurrentResponses>(CURR_KEY, {}); },
  save(r: CurrentResponses) { write(CURR_KEY, r); },
  clear() { if (isBrowser()) localStorage.removeItem(CURR_KEY); },
};