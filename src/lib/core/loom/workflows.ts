...
const poeticHandle = (seed: string) => {
  ...
  if (lowered.includes("burnout") || lowered.includes("overwhelm")) return "the quiet revolt against urgency";
  if (lowered.includes("ai") || lowered.includes("automation")) return "human signal in the machine weather";
  if (lowered.includes("launch") || lowered.includes("offer")) return "the threshold before the work becomes visible";
  if (lowered.includes("identity") || lowered.includes("voice")) return "the voice behind the visible work";
...
const makeRun = (
  ...
  const createdAt = now();
  const seed = asText(
    request.inputs.field ?? request.inputs.draft ?? request.inputs.source ?? request.inputs.topic ?? request.inputs.project ?? request.inputs.log,
    "Untitled thread",
  );
...
const signalCollapseHandler: LoomWorkflowHandler = {
  ...
  run: (request) => {
    const field = asText(request.inputs.field, "A scattered field is ready to become one clear thread.");
    ...
    return makeRun(
      ...
      request,
      [
        ...baseSteps("signal-collapse", "Separate signal from noise, then name the emotional and practical spine."),
        {
          id: "signal-collapse:angles",
          label: "Open usable angles",
          description: "Return creative handles, content angles, and a next artifact rather than only a summary.",
        },
      ],
      "Choose one content angle and make the smallest artifact that proves the signal is alive.",
      {
        visibleSignal: firstWords(field, 24),
        hiddenTension: "The work wants clarity, but too much certainty too early would flatten the living signal.",
        audienceResonance: "This may speak to people holding too much input who need one humane thread to follow.",
        creativeOpportunity: "Turn the pressure into a translation tool: show the shift from noise to usable pattern.",
        poeticHandle: poeticHandle(field),
        signalFragments: fragments,
        contentAngles: contentAngles(field),
        unexpectedBridge: "Treat the messy note like a weather map: not a problem to solve, but a pressure system to read.",
        nextArtifact: {
          type: "post",
  ...
  run: (request) => {
    ...
    return makeRun(
      ...
      "Draft the opening around the strongest headline before polishing any section.",
      {
        ...
        outline: ["Opening signal", "Hidden tension", "Three load-bearing beats", "Useful turn", "Closing invitation"],
        readerPromise: intent,
        texturePass: "Keep one vivid sentence, one practical sentence, and one sentence that names the emotional stakes.",
      },
    );
  },
};
...
const creativePersonasHandler: LoomWorkflowHandler = {
  ...
  run: (request) => makeRun(
    ...
    {
      personaSet: asText(request.inputs.voices, "curator, field guide, maker, skeptic").split(/,\s*/),
  ...
  run: (request) => {
    ...
    return makeRun(
      ...
      "Rewrite the first paragraph for the target platform while keeping the same promise.",
      {
        ...
        formatNotes: "Preserve the promise, change the shape: shorter hook, clearer turn, one action.",
...
const serendipityLabHandler: LoomWorkflowHandler = {
  ...
  run: (request) => makeRun(
    ...
    "Pick one combination and test it as a tiny creative experiment.",
    {
      ...
      strangeConnection: "Borrow structure from the adjacent world, not decoration. Ask what its rhythm teaches the work.",
...
const creativeOperatorHandler: LoomWorkflowHandler = {
  ...
  run: (request) => makeRun(
    ...
    "Do the first move only; do not open a new planning loop yet.",
    {
      ...
      stopRule: "Stop after the first artifact is saved; momentum counts only if it leaves a trace.",
    },
  ),
};
